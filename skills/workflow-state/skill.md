...
// Usage: Multi-step form processing
const stateManager = new WorkflowStateManager(firestoreStorage);

// Agent updates state during interaction
await stateManager.updateState(sessionId, (state) => ({
  ...state,
  step: 'collecting_email',
  data: {
    ...state.data,
    collectedFields: [...(state.data.collectedFields || []), 'email'],
    email: userInput
  }
}));
```
