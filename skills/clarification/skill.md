...
The classifier thinks the intent might be "${classification.intent_summary}" but is not confident.

Agent examples: ${agent.examples.join(', ')}

Generate a friendly, concise question that helps confirm what the user needs.`;
  }

  private getFallbackQuestion(agent: AgentConfig, language: string): string {
    const key = `${agent.id}:${language}`;
    return this.fallbackQuestions.get(key) || 
           this.fallbackQuestions.get(`${agent.id}:en`) ||
           `Can you please clarify what you need help with regarding ${agent.display_name}?`;
  }

  private loadFallbackQuestions(): Map<string, string> {
    // Pre-translated fallback questions for 45+ languages
    return new Map([
      ['password_reset:en', 'Are you trying to reset your password, or do you need help with something else?'],
      ['password_reset:es', '¿Estás intentando restablecer tu contraseña, or necesitas ayuda con otra cosa?'],
