...
interface Session {
  ...
  createdAt: string;
  updatedAt: string;
  ttl: string; // ISO timestamp for expiration
...
class SessionManager {
  ...
  async appendTurn(sessionId: string, entry: TurnEntry): Promise<void> {
    await this.store.transaction(sessionId, async (session) => {
      // Use atomic array union to append turn
  ...
  async updateWorkflowState(
    ...
    await this.store.transaction(sessionId, async (session) => {
      // Replace workflow state (not merge)
  ...
  async closeSession(sessionId: string, status: 'completed' | 'abandoned' | 'error'): Promise<void> {
    await this.store.update(sessionId, (session) => ({
      ...session,
      status,
      updatedAt: new Date().toISOString(),
      // Delete TTL field to allow Firestore TTL policy to GC the document
      ttl: null,
    }));
  }

  async resumeSession(priorSessionId: string, userId: string, agentId: string): Promise<Session> {
    // Get prior session's turn history
    const priorSession = await this.store.get(priorSessionId);
    
    // Create new session with carried-forward history
    const newSession = await this.createSession(userId, agentId);
    
    if (priorSession) {
      // Carry forward turn history (limit to prevent excessive size)
      const recentTurns = priorSession.turnHistory.slice(-50);
      await this.store.update(newSession.id, (session) => ({
        ...session,
        turnHistory: recentTurns,
        workflowState: priorSession.workflowState,
      }));
    }
